...
const BlogShowPage = ({ match, history, enqueueSnackbar }) => {
  const {
    params: { blogId },
  } = match;
  ...
  const toggleLike = () => {
    if (userAuth) {
      if (liked) {
        ...
        setBlog(currentBlog);
        likeBlog(blogId, blog.likes.filter((element) => element !== user.id))
        ...
        setBlog(currentBlog);
        likeBlog(blogId, [...blog.likes, user.id])
          .catch((err) => console.error(err));
      }
    } else {
      dispatch(SHOW_MODAL());
      ...
      {!loading ? (
        <div className="BlogShowPage">
          ...
          <div className="container row mx-auto" id="blog-card">
            ...
            <div className="cards col-lg-9 col-md-12" id={blogId}>
